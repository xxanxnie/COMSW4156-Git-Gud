...
std::string AuthService::registerUser(const std::string& email, 
    ...
    try {
        ...
        dbManager.findCollection(collection_name, query, result);
...
std::string AuthService::generateJWT(const User& user) {
    auto token = jwt::create()
        ...
        .set_expires_at(std::chrono::system_clock::now() + std::chrono::hours(JWT_EXPIRATION_HOURS))
...
bool authenticateToken(const crow::request& req) {
    ...
    static AuthService& authService = AuthService::getInstance(); // Assuming singleton pattern
...
bool AuthService::hasRole(const std::string& token, const std::string& requiredRole) {
