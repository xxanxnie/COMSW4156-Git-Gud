// according to https://github.com/mongodb-developer/get-started-cxx/blob/main/student-records-console-app/studentRecordsConsoleApp.cpp
#pragma once
...
bsoncxx::document::value createDocument(
    const vector<pair<string, string>>& keyValues) {
...
void insertDocument(mongocxx::collection& collection,
                    const bsoncxx::document::value& document) {
  collection.insert_one(document.view());
}

// Delete a document from a given collection.
void deleteDocument(mongocxx::collection& collection,
                    const bsoncxx::document::value& document) {
  collection.delete_one(document.view());
}

// Update the document with given key-value pair.
void updateDocument(mongocxx::collection& collection, const string& key,
                    const string& value, const string& newKey,
                    const string& newValue) {
  collection.update_one(
      bsoncxx::builder::stream::document{}
          << key << value << bsoncxx::builder::stream::finalize,
      bsoncxx::builder::stream::document{}
          << "$set" << bsoncxx::builder::stream::open_document << newKey
          << newValue << bsoncxx::builder::stream::close_document
          << bsoncxx::builder::stream::finalize);
}

// Find the document with given key-value pair.
void findDocument(mongocxx::collection& collection, const string& key,
                  const string& value) {
...
void inputStudentRecord(mongocxx::collection& collection) {
  ...
  insertDocument(collection, createDocument({{"name", name},
                                             {"rollNo", rollNo},
                                             {"branch", branch},
                                             {"year", year}}));
}
...
int main() {
  ...
  if (!(std::find(allCollections.begin(), allCollections.end(), collName) !=
        allCollections.end())) {
